...
import { approveReport, deleteReport, getReports } from "@/app/components/report/api";
...
export const All_reports = () => {
    ...
    const handleApprove = async (id: number) => {
        ...
        try {
            ...
            setReports(prev => prev.map(r => r.id === id ? { ...r, status: "Concluída" } : r));
        } catch (error) {
    ...
    const handleReject = async () => {
        ...
        try {
            ...
            setReports(prev => prev.map(r => r.id === selectedReportId ? { ...r, status: "Rejeitada" } : r));
        } catch (error) {
    ...
    return (
        <main className='w-full min-h-screen mt-[calc(8vh+1rem)] bg-white px-5 md:px-32 py-16'>
            <h3 className="text-clamp-xlarge text-[var(--font-black)] font-semibold mb-4">Denúncias em andamento</h3>
            {reports.length === 0 ? (
                ...
                <ul className="space-y-4">
                    {reports.map((report) => (
                        <li key={report.id} className="border p-8 rounded shadow">
                            <p className='text-clamp-large mb-4'><strong className='text-[var(--font-title)]'>Envio da denúncia</strong><br />{handleData(report.data_criacao)}</p>
                            ...
                            <strong className='text-[var(--font-title)] text-clamp-large'>Informações do usuário</strong>
                            {report.is_anonimo ? (
                                <p className='text-[var(--font-title)] text-clamp-large'>Anônimo</p>
                            ) : (
                                <div>
                                    <p className='truncate'><strong>Nome:</strong> {report.nome}</p>
                                    <p className='truncate'><strong>Sobrenome:</strong> {report.sobrenome}</p>
                                    <p className='truncate'><strong>E-mail:</strong> {report.email}</p>
                                    <p className='truncate'><strong>Telefone:</strong> {report.telefone}</p>
                                </div>
                            )}
                            <div className='mt-6 flex justify-evenly items-center gap-8 w-fit'>
                                ...
                                >
                                    <Check stroke='#fff' strokeWidth={3} className='inline -translate-y-px' /> Aprovar Denúncia
                                </button>
                                <button
                                    onClick={() => {
                                ...
                                >
                                    <CircleX stroke='#fff' strokeWidth={3} className='inline -translate-y-px' /> Rejeitar Denúncia
                                </button>
                            </div>
                        </li>
                    ))}
            ...
            {confirmRemoval && (
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center">
                    <dialog open className="p-8 rounded-2xl w-full max-w-md border-none shadow-[3px_4px_10px_#00000040] flex flex-col justify-between bg-white">
