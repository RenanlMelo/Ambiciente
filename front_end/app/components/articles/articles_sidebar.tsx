...
export const Articles_sidebar: React.FC<Props> = ({ article }) => {
  ...
  return (
    <>
      ...
      >
        <Funnel size={32} stroke="var(--white)" strokeWidth={2} />
      </button>
      ...
      {menuOpen && (
        <aside
          className={`md:hidden ${ibmPlexSans.className} overflow-y-scroll fixed bg-[var(--background)] w-screen p-8 flex flex-col gap-y-5 top-[calc(8vh+1rem)] bottom-0 box-border z-50`}
        >
          <button
            onClick={toggleMenu}
